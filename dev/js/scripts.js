...
app.map = function () {
  if ($('#js-map').length) {
    var image = '/' + window.location.href.split('/')[3] + '/img/company/marker.png';
    var myCenter = new google.maps.LatLng(35.69360222601097, 139.75995582452538);
...
app.tabs = function () {
  $('.js-tabs li').click(function () {
    ...
    if(!$('#' + tab).hasClass('is-show')){
      listTabs[$('#' + tab).attr('id')].min = $('#' + tab).find('.common-list-category').outerHeight() + $('.js-read-more:visible').height();
...
app.readMore = function () {
    ...
    tabs.each(function (index, item) {
      if ($(item).hasClass('current') && !$(item).hasClass('is-show')) {
        ...
        $(item)
          ...
          .removeClass('is-hide');
        var height2 = $(this).find('.common-list-category').outerHeight() + $('.js-read-more:visible').height();
  ...
  tabContent.each(function () {
    listTabs[$(this).attr('id')].min = $(this).find('.common-list-category').outerHeight() + $('.js-read-more:visible').height();
...
$(window).on('resize', function () {
  ...
  tabContent.each(function () {
    if($(this).find('.common-list-category').outerHeight() != 0 ){
      listTabs[$(this).attr('id')].min = $(this).find('.common-list-category').outerHeight() + $('.js-read-more:visible').height();
      $(this).css('height', listTabs[$(this).attr('id')].min);
    }else{
...
app.accordion = function () {
  $('.js-accordion')
    ...
    .on('click', function () {
      ...
      var height = $(this).parent().find('.js-accordion-content')[0].scrollHeight;

      if($(this).parent().hasClass('is-active-icon')) {
        $(this).parent().find('.js-accordion-content').css('height', height + 'px');
      } else {
        $(this).parent().find('.js-accordion-content').css('height', '0');
      }
    });
};
...
app.selectBox = function () {
  ...
  if (wrapSelect.length) {
    wrapSelect.click(function () {
      wrapSelect.not($(this)).find('.custom-select').removeClass('open').removeAttr('style');
